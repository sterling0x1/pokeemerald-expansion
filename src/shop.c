    ...
    if (itemId != LIST_CANCEL)
    {
        if (sMartInfo.martType == MART_TYPE_NORMAL)
        {
            ConvertIntToDecimalStringN(
                gStringVar1,
                Cheats_ApplyMartPrice(GetItemPrice(itemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT)),
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            if (sMartInfo.martType == MART_TYPE_NORMAL)
                sShopData->totalCost = Cheats_ApplyMartPrice(GetItemPrice(itemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT));
            else
            ...
            else
            {
                if (sMartInfo.martType == MART_TYPE_NORMAL)
                {
                    ...
                    if (GetItemImportance(itemId))
                    {
                        ...
                        tItemCount = 1;
                        sShopData->totalCost = Cheats_ApplyMartPrice(GetItemPrice(tItemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT)) * tItemCount;
