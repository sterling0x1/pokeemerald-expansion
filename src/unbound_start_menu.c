...
#define USM_SAVE_VERSION 1

struct Usm_SavedItems
{
    u8 items[USM_ICO_COUNT];
    u8 count;
};

static struct Usm_SavedItems *Usm_GetSavedItems(void)
{
    static EWRAM_DATA struct Usm_SavedItems sFallback;
    u16 version;
    u16 size;
    struct Usm_SavedItems *saved = ModuleSave_GetChunk(MODULE_ID_START_MENU_UI, &version, &size);

    if (saved == NULL || version != USM_SAVE_VERSION || size != sizeof(*saved))
        saved = ModuleSave_RecreateChunk(MODULE_ID_START_MENU_UI, USM_SAVE_VERSION, sizeof(*saved));

    if (saved != NULL && saved->count > USM_ICO_COUNT)
    ...
    if (saved != NULL)
    {
        for (u32 i = 0; i < saved->count; i++)
        {
            ...
            for (u32 j = 0; j < i; j++)
            {
                if (saved->items[i] == saved->items[j])
                {
                    memset(saved, 0, sizeof(*saved));
                    return saved;
                }
    ...
    return saved != NULL ? saved : &sFallback;
}
...
static const u32 sCheatsIconGfx[] = INCBIN_U32("graphics/unbound_start_menu/sprites/cheats.4bpp.smol");
...
static bool32 UsmMenuCB_Cheats(u32 state);
...
static const struct Usm_MenuItem sUsmMenuItems[USM_ICO_COUNT] = {
    ...
    [USM_ICO_CHEATS]  = USM_MENU_ITEM(Cheats),
};
...
static bool32 UsmMenuCB_Cheats(u32 state)
{
    ...
    default:
        if (!gPaletteFade.active)
        {
            ...
            gMain.savedCallback = CB2_ReturnToFieldWithOpenMenu;
            gMain.state = 0;
            SetMainCallback2(CB2_InitCheatMenu);
            return TRUE;
        }
    }
    return FALSE;
...
static const enum Usm_Icons sUsmDefaultItems[] = {
    ...
    USM_ICO_REST, USM_ICO_CHEATS, USM_ICO_OPTIONS, USM_ICO_RETIRE
};
...
{
    struct Usm_SavedItems *saved = Usm_GetSavedItems();
...
static bool32 Usm_IsItemAvailable(enum Usm_Icons item)
{
    switch (item) {
        ...
        case USM_ICO_CHEATS:  return MODULE_CHEATS_ENABLED;
        default:              return TRUE;
    }
}
...
static void Usm_HandleMainInput(void)
{
    ...
    if (JOY_NEW(SELECT_BUTTON))
    {
        ...
        if (sUsmState->move.handSpriteId == MAX_SPRITES)
        {
            sUsmState->mode = USM_MODE_NORMAL;
            Usm_AnimateSelectedIcon();
        }
...
static void Usm_SwapIconPos(u8 grabIndex, u8 targetIndex)
{
    u8 temp = sUsmState->items[grabIndex];
    sUsmState->items[grabIndex] = sUsmState->items[targetIndex];
    sUsmState->items[targetIndex] = temp;
...
static void Usm_ExitStartMenu(void)
{
    ...
    if (sUsmMemory->leftArrowId < MAX_SPRITES)
        DestroySprite(&gSprites[sUsmMemory->leftArrowId]);
    if (sUsmMemory->rightArrowId < MAX_SPRITES)
